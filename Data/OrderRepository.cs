...
public class OrderRepository(SqlConnectionFactory connectionFactory)
{
  ...
  public async Task<ConfirmOrderResponseDTO> ConfirmOrderAsync(int orderId)
  {
    ...
    ConfirmOrderResponseDTO confirmOrderResponseDTO = new()
    {
      OrderId = orderId,
    };
    using var connection = _connectionFactory.CreateConnection();
  ...
  {
    OrderStatusResponseDTO orderStatusDTO = new()
    {
      OrderId = orderId
    };
    using var connection = _connectionFactory.CreateConnection();
  ...
  private static bool IsValidStatusTransition(string currentStatus, string newStatus)
  {
    ...
    return currentStatus switch
    {
      ...
      "Confirmed" => newStatus == "Processing",
      "Processing" => newStatus == "Delivered",
      "Delivered" => false,// Delivered orders should not transition to any other status
      "Cancelled" => false,// Cancelled orders should not transition to any other status
      _ => false,
    };
  }
